...
function bindInteractions() {
  ...
  ui.sendBirdEye.addEventListener('click',async () => { ui.birdEyeStatus.textContent='Sending…'; try { const result=await api().sendBirdEye(); ui.birdEyeStatus.textContent=result.ok ? `BirdEye ${result.handoffId ? result.handoffId.slice(0,8) : 'sent'}` : result.error || 'BirdEye failed'; } catch (error) { ui.birdEyeStatus.textContent='BirdEye error'; ui.birdEyeStatus.title=error.message || String(error); } });
