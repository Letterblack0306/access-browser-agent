...
const ALLOWED_MODULES = Object.freeze({
  ...
  '../src/system/workbench-layout': { features: ['parseWorkbenchLayout'] },
...
const RUNTIME_MODULES = Object.freeze({
  ...
  'electron/rebuild-shell.js': contract('workbench-layout-owner', 'Own resizable workbench layout persistence and center/right/bottom tab visibility only.', 'Layout changes do not acquire task, provider, browser, or semantic agent authority.', 'Invalid layout interaction remains local UI state and cannot invent runtime success.', ['electron/preload.js']),
  'electron/rebuild-renderer.js': contract('workbench-controller', 'Own explicit user actions, passive boot, browser-loop controls, Stop All, and visible workbench state updates.', 'UI actions project backend truth and never auto-start browser/provider work during boot.', 'User-visible Problems/diagnostics surface failed actions without inventing success.', ['electron/preload.js','electron/rebuild-shell.js']),
  'electron/rebuild-runtime-state.js': contract('runtime-view-projection', 'Normalize backend snapshots/events into user-visible runtime, provider, loop, delivery, terminal, and problem state.', 'Visible state strength never exceeds backend evidence strength.', 'Unknown or ambiguous backend state stays unknown/unverified rather than guessed.', ['electron/rebuild-renderer.js']),
  'electron/rebuild-settings.js': contract('provider-settings-ui', 'Own explicit provider discovery, capability testing, provider activation, Chrome defaults, and integration settings.', 'Saved settings remain passive until the user explicitly invokes discovery/test/use/start actions.', 'Provider/settings errors remain visible and do not silently start runtime/browser work.', ['electron/rebuild-renderer.js']),
  'electron/rebuild-diagnostic-enhancer.js': contract('diagnostic-ui-enhancer', 'Enrich Complete Log correlation detail and show read-only module maintenance status in the Runtime view.', 'Maintenance drift appears as Caution/Maintained without gating runtime, browser, tools, or reasoning.', 'Status lookup failure appears as caution text only and never changes execution authority.', ['electron/rebuild-renderer.js']),
  ...
  'electron/rebuild-ide-reference.js': contract('ide-rail-delegate', 'Wire the 6 workbench activity-rail glyph buttons (data-ide-target) to their corresponding center/right tab clicks (data-center-tab, data-right-tab) and mirror is-active/aria-pressed state.', 'Every glyph click switches the visible center or right tab panel to the bound view (Files, Agent, Execution, Editor, Browser loop, Runtime, Settings) and updates the rail visual state; tab clicks in the center/right update the rail in turn.', 'Missing rail markup, missing target tab, or duplicate event handlers fall through without invoking runtime, browser, or provider owners.', ['electron/rebuild-shell.js']),
});
